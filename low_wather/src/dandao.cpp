...
void KAL2::sp_reset(kal_filter& kf)
{
    kf.Xk_1[1] = 0.0001;
    kf.Xk_1[2] = 0.001;
...
bool KAL2::predict(RotatedRect& detection, kal_filter& kf, double time)
{
    //t=-1;
    ...
    Eigen::Vector3d m_pc = pnp_get_pc(pts, w, h);  // T_vec
/*
    if (depth > 5.8)
    {
        ///--------------  ??????
...
*/
    ...
    if (t == -1)
    {
        ...
        double dis;
        double angle_pitch = get_pitch(m_pd, dis);
        get_send(m_pd, angle_pitch, dis);
        kf.Xk_1[0] = m_pd(0, 0);
        kf.Xk_1[3] = m_pd(1, 0);
...
//		last_aim_yaw = ra_yaw - ab_yaw;
    ...
    double predict_time = m_pd.norm() / SPEED + shoot_delay;  //pd  not pc
    //printf("speed:%lf\n",SPEED);
    //printf("pre_time:%lf\n",predict_time);
    ...
    shoot_delay = (fabs(pre_xy[1]) / 1.8) * shoot_delay_init;  //  delete xishu


    Eigen::Vector3d pos3 = { pre_xy[0],pre_xy[3],m_pd(2,0) };;
    pos3 = rotated_matrix.inverse() * pos3;   ///   rotated_matrix.transpose() = rotated_matrix.inverse()
...
//	send.pitch = atan2(pos3(1, 0) - 0.055 - height *  xishu, pos3(2, 0))/PI * 180.0 - ab_pitch;
    double dis;
    double angle_pitch = get_pitch(m_pd, dis);
    printf("---angle_pitch: %lf\n", angle_pitch);
    get_send2(m_pd, angle_pitch, dis);
    return true;

}

Eigen::Vector3d KAL2::pnp_get_pc(const cv::Point2f p[4], const double& w, const double& h)
{
...
float KAL2::keep_pi(float angle)
{
    if (angle > 180.0)
    {
        return -(360.0 - angle);
...
void KAL2::depth_filter(std::deque<double>& dis)
{
